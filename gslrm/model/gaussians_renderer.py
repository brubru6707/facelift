...
def render_turntable(pc: GaussianModel, rendering_resolution=384, num_views=8):
    ...
    _n_gaussians = pc._xyz.shape[0]
    _mem_alloc = torch.cuda.memory_allocated() / 1024**3
    _mem_reserved = torch.cuda.memory_reserved() / 1024**3
    _mem_total = torch.cuda.get_device_properties(device).total_memory / 1024**3
    print(f"[DEBUG render_turntable] Starting with {_n_gaussians:,} Gaussians, "
          f"resolution={rendering_resolution}x{rendering_resolution}, num_views={num_views}")
    print(f"[DEBUG render_turntable] GPU memory: {_mem_alloc:.2f}/{_mem_total:.2f} GiB allocated, "
          f"{_mem_reserved:.2f} GiB reserved")

    # --- Scale stats ---
    _rt_scales = pc.get_scaling.max(dim=1).values
    print(f"[DEBUG render_turntable] Scale stats: min={_rt_scales.min().item():.4f}  "
          f"max={_rt_scales.max().item():.4f}  mean={_rt_scales.mean().item():.4f}")

    # --- xyz NaN/Inf ---
    _rt_nan = torch.isnan(pc._xyz).any().item()
    _rt_inf = torch.isinf(pc._xyz).any().item()
    print(f"[DEBUG render_turntable] xyz NaN={_rt_nan}  Inf={_rt_inf}")

    # --- Rotation sanity: zero-norm before normalize causes NaN quaternion in CUDA ---
    _rt_rot_raw = pc._rotation
    _rot_norms = _rt_rot_raw.norm(dim=1)
    _zero_rot = (_rot_norms < 1e-8).sum().item()
    _rot_raw_nan = torch.isnan(_rt_rot_raw).any().item()
    _rot_raw_inf = torch.isinf(_rt_rot_raw).any().item()
    print(f"[DEBUG render_turntable] _rotation raw: NaN={_rot_raw_nan}  Inf={_rot_raw_inf}  "
          f"zero-norm count={_zero_rot}  norm range=[{_rot_norms.min().item():.4f}, {_rot_norms.max().item():.4f}]")
    _normed_rot = pc.get_rotation  # after normalize()
    _normed_nan = torch.isnan(_normed_rot).any().item()
    _normed_inf = torch.isinf(_normed_rot).any().item()
    print(f"[DEBUG render_turntable] get_rotation (normalized): NaN={_normed_nan}  Inf={_normed_inf}")

    # --- Camera params and per-Gaussian view-space depth for view 0 ---
    # This is the ground truth: what depth does each Gaussian actually appear at in the rasterizer?
    _cam0 = Camera(C2W=c2w[0], fxfycxcy=fxfycxcy[0], h=h, w=w)
    print(f"[DEBUG render_turntable] View 0 tanfovX={_cam0.tanfovX:.6f}  tanfovY={_cam0.tanfovY:.6f}")
    print(f"[DEBUG render_turntable] View 0 cam_center: {[round(x, 4) for x in _cam0.camera_center.tolist()]}")
    _xyz_h = torch.cat([pc._xyz, torch.ones(_n_gaussians, 1, device=device)], dim=1)  # [N, 4]
    _xyz_c = (_cam0.W2C @ _xyz_h.T).T  # [N, 4]
    _z_c = _xyz_c[:, 2]
    _near_culled = (_z_c <= 0.2).sum().item()
    print(f"[DEBUG render_turntable] View 0 view-space z: min={_z_c.min().item():.4f}  "
          f"max={_z_c.max().item():.4f}  mean={_z_c.mean().item():.4f}  "
          f"near-culled(z<=0.2)={_near_culled:,}")

    # --- Compute per-Gaussian projected radius and estimated tiles_touched for view 0 ---
    # This directly predicts what num_rendered will be in the CUDA kernel.
    _focal_x = w / (2.0 * _cam0.tanfovX)
    _in_front = (_z_c > 0.2)
    _n_in_front = _in_front.sum().item()
    _z_front = _z_c[_in_front].clamp(min=1e-6)
    _s_front = _rt_scales[_in_front]
    _proj_r = (_focal_x * _s_front / _z_front)  # 2D spread in pixels (1-sigma)
    _my_radius = (3.0 * _proj_r).ceil()          # same formula as CUDA forward.cu
    _tiles_w = (2.0 * _my_radius / 16 + 1).clamp(max=32).long()
    _tiles_est = (_tiles_w * _tiles_w).sum().item()
    print(f"[DEBUG render_turntable] View 0 in-front Gaussians: {_n_in_front:,}")
    print(f"[DEBUG render_turntable] View 0 proj_radius(px): min={_proj_r.min().item():.2f}  "
          f"max={_proj_r.max().item():.2f}  mean={_proj_r.mean().item():.2f}")
    print(f"[DEBUG render_turntable] View 0 my_radius(px): min={_my_radius.min().item():.0f}  "
          f"max={_my_radius.max().item():.0f}  mean={_my_radius.mean().item():.1f}")
    _oversize = (_my_radius > 512).sum().item()
    print(f"[DEBUG render_turntable] View 0 my_radius > 512px (likely overflow risk): {_oversize:,}")
    print(f"[DEBUG render_turntable] View 0 estimated tiles_touched sum: {_tiles_est:,.0f}  "
          f"(int32 max={2**31-1:,})")
    print(f"[DEBUG render_turntable] View 0 BinningState estimate: "
          f"{_tiles_est * 24 / 1024**3:.3f} GiB  (3.7 GiB = OK, >23 GiB = crash)")
    del _xyz_h, _xyz_c, _z_c, _z_front, _s_front, _proj_r, _my_radius, _tiles_w

    renderings = torch.zeros(v, 3, h, w, dtype=torch.float32, device=device)
    for j in range(v):
        if j == 0 or j % 10 == 0:
            _m = torch.cuda.memory_allocated() / 1024**3
            print(f"[DEBUG render_turntable] Rendering view {j}/{v}, GPU mem: {_m:.2f} GiB allocated")
        # Enable debug=True for view 0 to save snapshot_fw.dump if crash occurs.
        # The snapshot contains all input tensors (CPU copy) for offline analysis.
        _debug_view = (j == 0)
        try:
            renderings[j] = render_opencv_cam(pc, h, w, c2w[j], fxfycxcy[j], debug=_debug_view)["render"]
        except RuntimeError as e:
            _m_fail = torch.cuda.memory_allocated() / 1024**3
            _r_fail = torch.cuda.memory_reserved() / 1024**3
            print(f"[DEBUG render_turntable] CRASH at view {j}/{v} with {_n_gaussians:,} Gaussians — "
                  f"GPU: {_m_fail:.2f} GiB alloc / {_r_fail:.2f} GiB reserved — error: {e}")
            if j == 0:
                print("[DEBUG render_turntable] snapshot_fw.dump saved in CWD for offline analysis")
            raise
    ...
    print(f"[DEBUG render_turntable] Done. All {v} views rendered successfully.")
    return renderings
