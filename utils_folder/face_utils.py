...
def crop_face(
    ...
    print(f"[TIMING]   MTCNN face detection: {time.time() - t0:.2f}s")
    ...
    print(f"[TIMING]   crop/resize/paste: {time.time() - t0:.2f}s")
...
def prepare_foreground_with_rembg(input_image_array: np.ndarray) -> np.ndarray:
    ...
    t0 = time.time()
    background_removed_image = remove(pil_image, session=REMBG_SESSION)
    print(f"[TIMING]   rembg background removal: {time.time() - t0:.2f}s")
